...
| `assets/styles/*.css` | テンプレートに埋め込むCSS群（theme-vars / slide-core / nav-controls / figure / **chart** / list-view の順で統合・全6ファイル） |
...
6. ユーザーの確認・修正後、確定したフレーズと画像方針を記録して Step 2 へ進む
---
## Step 2: Web検索・情報収集
...
### 画像 URL の収集（「画像あり」選択時のみ）

Step 1-C で「画像あり」が選択された場合のみ実行する。「テキストのみ」の場合はスキップ。
...
2. `assets/styles/` の6ファイルを `<style>` に統合する（theme-vars → slide-core → nav-controls → figure → **chart** → list-view の順）
...
6. `assets/scripts/` の4ファイルを `<script>` に統合する（fit-slide → theme-toggle → view-toggle → navigation の順）
### 機能チェック
...
- [ ] 非数値テーマ・会話のみモードで、グラフが過剰生成されていないか
